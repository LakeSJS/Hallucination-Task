...
// Digital OUT
#define PIN_HOUSE_LAMP     34  // House Lamp Pin         (DUE = 34)  (MEGA = 34)  (UNO = 5?)  (TEENSY = 6)	(TEENSY3.4 6)
#define PIN_LED_CUE        28  // Cue LED Pin            (DUE = 35)  (MEGA = 28)  (UNO =  4)  (TEENSY = 4)	(TEENSY3.4 4)
#define PIN_NOISE          29  // LED representing noise - replace with speaker in full version of code
#define PIN_LEFT_REWARD    52  // Left Reward Pin             (DUE = 37)  (MEGA = 52)  (UNO =  7)  (TEENSY = 7)	(TEENSY3.4 7)
#define PIN_RIGHT_REWARD   53  // Right Reward Pin                   (MEGA = 53)
...
// Digital IN
#define PIN_RIGHT_LICK     2    // Right Lick Pin         (DUE = 36)  (MEGA =  2)  (UNO =  2)  (TEENSY = 2)	(TEENSY3.4 22)
#define PIN_LEFT_LICK      3    // Left Lick Pin
#define PIN_RECEIPT        8  	// Confirms Optogenetics Command Received 					   (TEENSY = 8)	(TEENSY3.4 8)



// Legacy:
#define PIN_SHOCK          22  // Shock Pin*** modified bc line 1 for first lick (MEGA = 22)   (TEENSY = 13) (TEENSY3.4 13)
...
//PWM OUT:
#define PIN_QUININE        9   // Quinine Pin            (DUE = 22)  (MEGA =  9)  (UNO =  8)  (TEENSY = 14) (TEENSY3.4 14)** Must be PWM
...
enum State
{
	...
	NOISE_TRIAL,          // Play white noise, no cue
  CUE_TRIAL,            // Play white noise with cue
	RESPONSE_WINDOW,      // Prompt LED ON, Check for licks
	REWARD_LEFT,          // Dispense left reward, wait for trial Timeout
  REWARD_RIGHT,         // Dispense right reward, wait for trial Timeout
		...
		which can be parsed later to validate time measurements */
{
	...
	EVENT_LEFTlja_REWARD,      // Left reward dispensed
  EVENT_RIGHT_REWARD      // Right reward dispensed
...
static long _cue_dice_roll           = 0;        // Randomly select if trial will have cue present
static bool _is_cued                 = true;     // Track if current trial is cued
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	INIT_TRIAL - Trial started. House Lamp OFF
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void init_trial() {
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		ACTION LIST -- initialize the new state
    ...
    /*---------Decide if trial will be Noise alone or Cue----------*/
    _cue_dice_roll = random(1,100);
    if (_cue_dice_roll <= _params[PERCENT_CUE])
    {
      _is_cued = true; // set this trial as cued
    } else {
      _is_cued = false //set this trial as noise only, no cue
    }
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	CUE_TRIAL - Timing Interval (Cue presentation to opening of Reward Response Window for trials with cue)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void cue_trial() {
  
  
  if (_state!= _prevState) {
    // TODO wait a random delay
    
    digitalWrite(PIN_NOISE,HIGH); // play white noise
    noise_on_timestamp = signedMillis();
    
  } else { // if the previous state is also cue_trial
    noise_on_time = signedMillis() - noise_on_timestamp;
    if ( (noise_on_time >= cueOnset) && (noise_on_time < noiseDuration) ) { // if it is time to play the cue and still in white noise window
      digitalWrite(PIN_CUE_LED,HIGH); // cue on
    } else {
      digitalWrite(PIN_CUE_LED,LOW); // cue off
    }
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	RIGHT_REWARD - Deliver reward and wait for trial timeout while tracking licks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void right_reward() {
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		ACTION LIST -- initialize the new state
	...
	if (_state != _prevState) {                        // If ENTERTING REWARD:
		...
		setReward(true,'R');                                    // Initiate reward delivery
		...
		//------------------------DEBUG MODE--------------------------//  
		if (_params[_DEBUG]) {sendMessage("Dispensing right reward.");}
		//----------------------end DEBUG MODE------------------------//
	}
	...
	if (signedMillis() - _reward_timer >= _params[REWARD_DURATION] && !_reward_dispensed_complete) { // Reward duration elapsed...terminate reward
		setReward(false,'R');                                   // Stop delivery
		_reward_dispensed_complete = true;                  // track completion
		if (_params[_DEBUG]) {
			sendMessage("right reward terminated at " + String(signedMillis() - _reward_timer) + "ms wrt reward initiation.");
		}
	}
	...
	_state = RIGHT_REWARD;                                  // No Command --> Cycle back to REWARD
} // End RIGHT_REWARD STATE ------------------------------------------------------------------------------------------------------------------------------------------

/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	LEFT_REWARD - Deliver reward and wait for trial timeout while tracking licks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void left_reward() {
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		ACTION LIST -- initialize the new state
	...
	if (_state != _prevState) {                        // If ENTERTING REWARD:
		_reward_timer = signedMillis();                        // Start _reward_timer
		setReward(true,'L');                                    // Initiate reward delivery
		playSound(TONE_REWARD);                             // Start reward tone    
		// Send event marker (reward) to HOST with timestamp
		sendMessage("&" + String(EVENT_REWARD) + " " + String(signedMillis() - _exp_timer));
		if (_params[HYBRID] == 0) {                      // If NOT hybrid trial:
	...
	if (signedMillis() - _reward_timer >= _params[REWARD_DURATION] && !_reward_dispensed_complete) { // Reward duration elapsed...terminate reward
		setReward(false,'L');                                   // Stop delivery
		_reward_dispensed_complete = true;                  // track completion
		if (_params[_DEBUG]) {
	...
	_state = LEFT_REWARD;                                  // No Command --> Cycle back to LEFT_REWARD
} // End LEFT_REWARD STATE ------------------------------------------------------------------------------------------------------------------------------------------
...
void intertrial() {
	...
	if (_state != _prevState) {                     // If ENTERTING ITI:
		...
		setCueLED(false);                                // Cue LED OFF
		setReward(false,'R');                            // Stop right reward if still going
    setReward(false,'L');                            // Stop left reward if still going
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	Set noise LED (ON/OFF)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void setNoiseLED(bool turnOn) {
	if (turnOn) {
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void setReward(bool turnOn,char spoutChoice) {
	if (turnOn)                                                       
	{ 
    if (spoutChoice == 'R')
    {                                                             // MOUSE: Deliver Reward = TRUE
		  digitalWrite(PIN_RIGHT_REWARD, HIGH);                         // Right Reward Pin HIGH
		//------------------------DEBUG MODE--------------------------//  
			// if (_params[_DEBUG]) {sendMessage("Dispensing Reward");}
		//----------------------end DEBUG MODE------------------------//
    }
    if (spoutChoice == 'L')
    {
      digitalWrite(PIN_LEFT_REWARD, HIGH);                          // Left Reward Pin High
    }        
	}
	else
	{ 
    if (spoutChoice == 'R')
    {                                                                 // MOUSE: Stop Right Reward
		  digitalWrite(PIN_RIGHT_REWARD, LOW);                            // Right Reward Pin LOW
		}
    if (spoutChoice == 'L')
    {                                                                 // MOUSE: Stop Left Reward
      digitalWrite(PIN_LEFT_REWARD, LOW);                             // Left Reward Pin LOW
    }
    //------------------------DEBUG MODE--------------------------// 
