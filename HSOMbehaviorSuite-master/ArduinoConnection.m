...
% ------------------------------------------------------------------------------------
        ...
        function initializeUML(obj)
            ...
            par.x0 = 500;    % the initial signal strength
            par.x_lim = [0 1000];   % the limits to the signal strength
            ...
            par.lambda = struct(...
                'limits',[0 0.02],...      %range of the parameter space for lambda
        ...
        function findNewSNR(obj)
            ...
            SNRind = find(strcmp(obj.ParamNames,'SNR_PERMILLE'));
            resultCode = obj.Trials(end).Code;
            r = resultCode == 1;
            UMLobj.update(r);
            xnext = round(UMLobj.xnext);
            obj.SetParam(SNRind,xnext);
            assignin("base","UMLobj",UMLobj);
            
            fsigm = @(param,xval) param(1)+(param(2)-param(1))./(1+10.^((param(3)-xval)*param(4)));
            fitparams = sigm_fit([0; 1000; UMLobj.x],[0; 1; UMLobj.r],[0 1 nan nan],0);
            %mdl = fitglm(x, y, "Distribution", "binomial");
            
            xreg = linspace(0,1000,1000)'; % test data
            %yreg = predict(mdl,xreg);
            yreg = fsigm(fitparams,xreg);
            figure
            cla
            hold on
            plot(UMLobj.x,UMLobj.y,'o')
            plot(xreg,yreg)           
        end
