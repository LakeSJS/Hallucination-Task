...
static const int _stateDurationsMS[] = {
    1000, // _INIT
    1000, // IDLE_STATE
    1000, // INIT_TRIAL
    1000, // CUE_TRIAL
...
static const char *_paramNames[] =
    {
        ...
        "PENALTY_DURATION_MS",
        "TIME_NOISE_START",
        "MOVING_AVG_WINDOW"}; //**** BE SURE TO INIT NEW PARAM VALUES BELOW!*****//
...
void loop()
{
    ...
    while (true)
    {
        ...
        {                                // If there's something in the SERIAL INPUT BUFFER (i.e., if another character from host is waiting in the queue to be read)
            char inByte = Serial.read(); // Read next character

            // The pound sign ('#') indicates a complete message!------------------------
            if (inByte == '#')
            { // If # received, terminate the message
...
void mySetup()
{
    ...
    // state timers - track how long controller has been in each state
     _timer_init_trial = 0;      // Tracks time in init_trial state
     _timer_cue_trial = 0;       // Tracks time in cue_trial state
     _timer_noise_trial = 0;     // Tracks time in noise_trial state
     _timer_response_window = 0; // Tracks time in response_window state
     _timer_hit = 0;             // Tracks time in hit state
     _timer_miss = 0;            // Tracks time in miss state
     _timer_correct_reject = 0;  // Tracks time in correct_reject state
     _timer_false_alarm = 0;     // Tracks time in false_alarm state
     _timer_no_response = 0;     // Tracks time in no_response state
     _timer_intertrial = 0;      // Tracks time in intertrial state

    // number trackers for trials completed
     _num_trials = 0;          // Tracks number of trials completed
     _num_left = 0;            // Tracks number of trials where animal licked left
     _num_right = 0;           // Tracks number of trials where animal licked right
     _num_correct = 0;         // Tracks number of correct trials
     _num_incorrect = 0;       // Tracks number of incorrect trials
     _num_hits = 0;            // Tracks number of hits
     _num_misses = 0;          // Tracks number of misses
     _num_correct_rejects = 0; // Tracks number of correct rejects
     _num_false_alarms = 0;    // Tracks number of false alarms
     _num_no_responses = 0;    // Tracks number of no responses

    // percent trackers for animal performance
     _percent_left = 0;           // Tracks percent of trials that the animal chose the left side
     _percent_right = 0;          // Tracks percent of trials that the animal chose the right side
     _percent_correct = 0;        // Tracks percent of trials that the animal chose the correct side
     _percent_incorrect = 0;      // Tracks percent of trials that the animal chose the incorrect side
     _percent_hit = 0;            // Tracks percent of trials that the animal got a hit
     _percent_miss = 0;           // Tracks percent of trials that the animal missed
     _percent_correct_reject = 0; // Tracks percent of trials that the animal correctly rejected
     _percent_false_alarm = 0;    // Tracks percent of trials that the animal had a false alarm
     _percent_no_response = 0;    // Tracks percent of trials that the animal did not respond in time

    // moving average trackers for animal performance
     _recentHitRate = 0;           // Tracks the hit rate over the last 6 trials
     _recentMissRate = 0;          // Tracks the miss rate over the last 6 trials
    ...
    // lick trackers
     ...
     _leftLick = false;       // True when left lick detected, False when no lick
     _rightLick = false;      // True when right lick detected, False when no lick
     _prevLeftLick = false;   // lags behind _leftLick by one loop, used to detect when new lick begins
     _prevRightLick = false;  // lags behind _rightLick by one loop, used to detect when new lick begins
     _toneSpoutLick = false;  // True when tone spout lick detected, False when no lick
     _noiseSpoutLick = false; // True when noise spout lick detected, False when no lick
     _is_S2 = false;          // Set during INIT_TRIAL state, indicates whether trial will have S2 present or be noise only
...
void idle_state()
{
    ...
    if (_command == 'G')
    { // If Received GO signal from HOST ---transition to---> READY
...
void init_trial()
{
    ...
    if (signedMillis() - _timer_init_trial >= _stateDurationsMS[_state])
    { // Init_trial time allotted has elapsed -> White noise stimulus window
        if (_params[_DEBUG])
        {
...
void noise_trial()
{
    ...
    {                                      // If ENTERING NOISE_TRIAL STATE:
        ...
        if (_params[_DEBUG])
        {
            sendMessage("Noise Trial Started.");
        }
        // Do the initial business of the state -- something you only want to do once when you enter
    }
    ...
    // define condition for leaving this state
    if (time_elapsed >= (_params[TIME_NOISE_START] + _params[WHITE_NOISE_DURATION_MS]))
    { // noise_trial time allotted has elapsed -> response window
        if (_params[_DEBUG])
        {
            sendMessage("white noise trial successfully completed.");
        }
...
void cue_trial()
{
    ...
    if (isWithinNoiseWindow)
    { // if it is within time window to play noise
        if (isWithinS2Window)
        {                          // if it is within time window to play S2
    ...
    // define condition for leaving this state
    if (time_elapsed >= (_params[TIME_NOISE_START] + _params[WHITE_NOISE_DURATION_MS]))
    { // cue_trial time allotted has elapsed -> response window
        if (_params[_DEBUG])
        {
            sendMessage("cue trial successfully completed.");
        }
        _state = RESPONSE_WINDOW; // Move to RESPONSE_WINDOW state
        return;                   // Exit Fx
    }
...
void response_window()
{
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    if (_state != _prevState)
    { // If ENTERING RESPONSE_WINDOW STATE:
    ...
    if ((_leftLick && _params[S2_SPOUT] == 1) || (_rightLick && _params[S2_SPOUT] == 2))
    { // if the animal licked the S2 spout
        _toneSpoutLick = true;
        _noiseSpoutLick = false;
        // if debugging, send message to HOST that the animal licked the tone spout
        if (_params[_DEBUG])
        {
            sendMessage("Animal licked the tone spout.");
        }
    }
    if ((_leftLick && _params[S2_SPOUT] == 2) || (_rightLick && _params[S2_SPOUT] == 1))
    { // if the animal licked the noise spout
        _toneSpoutLick = false;
        _noiseSpoutLick = true;
        // if debugging, send message to HOST that the animal licked the noise spout
        if (_params[_DEBUG])
        {
            sendMessage("Animal licked the noise spout.");
        }
    }
    // define condition for leaving this state
    if (_is_S2 && _toneSpoutLick)
    {                     // if S2 was played and the animal licked the S2 spout
        _state = HIT;     // send to HIT state
        setCueLED(false); // turn off cue LED
        return;
    }
    if (_is_S2 && _noiseSpoutLick)
    {                     // if S2 was played and the animal licked the noise spout
        _state = MISS;    // send to MISS state
        setCueLED(false); // turn off cue LED
        return;
    }
    if (!_is_S2 && _toneSpoutLick)
    {                         // if S2 was not played and the animal licked the S2 spout
        _state = FALSE_ALARM; // send to FALSE_ALARM state
        setCueLED(false);     // turn off cue LED
        return;
    }
    if (!_is_S2 && _noiseSpoutLick)
    {                            // if S2 was not played and the animal licked the noise spout
        _state = CORRECT_REJECT; // send to CORRECT_REJECT state
        setCueLED(false);        // turn off cue LED
        return;
    }
    if (signedMillis() - _timer_response_window >= _params[RESPONSE_WINDOW_MS])
    { // response window time has been exceeded
        if (_params[_DEBUG])
        {
            sendMessage("mouse did not lick within response window.");
        }
        setCueLED(false);     // turn off cue LED
...
void false_alarm()
{
    ...
    {                                      // If ENTERING FALSE_ALARM STATE:
        ...
        if (_params[_DEBUG])
        {
            sendMessage("mouse false alarmed.)");
        }
...
void correct_reject()
{
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    if (_state != _prevState)
    {                                      // If ENTERING CORRECT_REJECT STATE:
        ...
        if (_params[_DEBUG])
        {
            sendMessage("mouse correctly rejected.)");
        }
...
void no_response()
{
    ...
    {                                      // If ENTERING NO_RESPONSE STATE:
        ...
        if (_params[_DEBUG])
        {
            sendMessage("mouse did not respond.)");
        }
    ...
    if (signedMillis() - _timer_no_response > _params[PENALTY_DURATION_MS])
    {                        // if penalty time has elapsed
        _state = INTERTRIAL; // go to intertrial state
        return;
    }
...
void intertrial()
{
    ...
    if (_command == 'O')
    {                              // HOST transmission complete: HOST sends 'O' for Over.
    ...
    if (signedMillis() - _timer_intertrial >= _params[ITI_DURATION_MS] && (isParamsUpdateDone || !isParamsUpdateStarted))
    {                        // End when ITI ends. If param update initiated, should also wait for update completion signal from HOST ('O' for Over).
...
void updatePerformance()
{
    ...
    if (_state == HIT)
    { // animal licked tone spout when tone was played
        _num_hits++;
        _num_correct++;
        if (_params[S2_SPOUT] == 1)
        {                // if tone spout is on the left
            _num_left++; // mark that the animal licked the tone spout (left)
        }
        else if (_params[S2_SPOUT] == 2)
        {                 // if tone spout is on the right
            _num_right++; // mark that the animal licked the tone spout (right)
        }
    }
    else if (_state == MISS)
    { // animal licked noise spout when tone was played
        _num_misses++;
        _num_incorrect++;
        if (_params[S2_SPOUT] == 1)
        {                 // if tone spout is on the left
    ...
    else if (_state == FALSE_ALARM)
    { // animal licked tone spout when noise was played
        _num_false_alarms++;
        _num_incorrect++;
        if (_params[S2_SPOUT] == 1)
        {                // if tone spout is on the left
    ...
    else if (_state == CORRECT_REJECT)
    { // animal licked noise spout when noise was played
...
void getLickState()
{
    if (digitalRead(PIN_LEFT_LICK) == HIGH)
    {
        _leftLick = true;
    }
    else
    {
        _leftLick = false;
    } // update left lick global variable
    if (digitalRead(PIN_RIGHT_LICK) == HIGH)
    {
        _rightLick = true;
    }
    else
    {
        _rightLick = false;
    } // update right lick global variable
} // end Get Lick State---------------------------------------------------------------------------------------------------------------------
...
double logistic_regression(const std::vector<double>& x, const std::vector<int>& y, int num_iterations) {
    double beta_0 = 0;
    double beta_1 = 0.5;
    double learning_rate = 0.025;
    // get the length of vector x
    int n = x.size();
    // send message to host indicating value of n
    sendMessage("n = " + String(n));

    for (int i = 0; i < num_iterations; i++) {
        for (int j = 0; j < n; j++) {
            double log_odds = beta_0 + beta_1 * x[j];
            double prediction = sigmoid(log_odds);
            double error = y[j] - prediction;
            beta_0 += learning_rate * error * prediction * (1 - prediction);
            beta_1 += learning_rate * error * prediction * (1 - prediction) * x[j];
        }
    }
    // find value of x where y = 0.5
    double x50 = - beta_0 / beta_1;

    // send message to host with new x50 value
    sendMessage("x50 = " + String(x50));
    return x50;
}
